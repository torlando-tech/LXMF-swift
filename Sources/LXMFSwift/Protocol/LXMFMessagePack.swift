//
//  LXMFMessagePack.swift
//  LXMFSwift
//
//  Simple MessagePack encoding/decoding for LXMF wire format.
//  Provides low-level control for exact wire compatibility with Python LXMF.
//
...
/// MessagePack value type for LXMF encoding/decoding.
///
/// Supports the subset of MessagePack types used by LXMF messages.
public enum LXMFMessagePackValue: Equatable, Hashable {
    case null
    case bool(Bool)
    case int(Int64)
    case uint(UInt64)
    ...
    case string(String)
    case binary(Data)
    case array([LXMFMessagePackValue])
    case map([LXMFMessagePackValue: LXMFMessagePackValue])
}
...
public func packLXMF(_ value: LXMFMessagePackValue) -> Data {
    var data = Data()
    encodeValue(value, to: &data)
    return data
}

private func encodeValue(_ value: LXMFMessagePackValue, to data: inout Data) {
    switch value {
    case .null:
        data.append(0xc0)
...
private func decodeString(length: Int, from data: Data, at offset: inout Int) throws -> LXMFMessagePackValue {
    ...
    guard let string = String(data: Data(bytes), encoding: .utf8) else {
        throw LXMFError.decodingFailed("Invalid UTF-8 string")
    }
    return .string(string)
}

private func decodeBinary(length: Int, from data: Data, at offset: inout Int) throws -> LXMFMessagePackValue {
    guard offset + length <= data.count else {
        throw LXMFError.decodingFailed("Unexpected end of data")
    }
